...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        ...
        if (weatherService == null) {
            weatherService = new RestAdapter.Builder()
                    .setEndpoint(FORECAST_BASE_URL)
                    .setLogLevel(RestAdapter.LogLevel.FULL)
                    .build()
                    .create(WeatherService.class);
        }

        Forecast forecast = weatherService.getForecast(locationQuery, format, units, Integer.toString(numDays));
        Forecast.City city = forecast.getCity();
        long locationId = addLocation(locationQuery, city.getName(), city.getCoord().getLat(), city.getCoord().getLon());

        List<Forecast.DayForecast> nextDays = forecast.getList();
        // Insert the new weather information into the database
        Vector<ContentValues> cVVector = new Vector<ContentValues>(nextDays.size());
        for (Forecast.DayForecast dayForecast : nextDays) {

            ContentValues weatherValues = new ContentValues();

            weatherValues.put(WeatherEntry.COLUMN_LOC_KEY, locationId);
            weatherValues.put(WeatherEntry.COLUMN_DATETEXT, WeatherContract.getDbDateString(new Date(dayForecast.getDt() * 1000L)));
